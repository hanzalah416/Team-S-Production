...
export function ServiceRequestGetter() {
    ...
    const updateFlowerRequestStatus = async (orderNumber: number, newStatus: string) => {
        try {
            await axios.patch(`/api/flower-request/${orderNumber}`, { status: newStatus });
            setFlowerRequestData((prevData) =>
                prevData.map((request) =>
                    request.orderNumber === orderNumber ? { ...request, status: newStatus } : request
                )
            );
        } catch (error) {
                    ...
                    <TableBody>
                        {sortedFlowerRequestData.map((flowerform) => (
                            <ServiceRequestDisplay
                                ...
                                onUpdateStatus={(newStatus) => updateFlowerRequestStatus(flowerform.orderNumber, newStatus)}
                            />
                        ))}
                    </TableBody>
                </Table>
